...
if not os.path.exists(output_folder):
    ...
    if not os.path.exists(file_path):
        ...
        fir_coeff = signal.firwin(numtaps, lowcut, window='boxcar', fs=sr)

        # Terapkan filter FIR pada sinyal
        filtered_signal_fir = signal.lfilter(fir_coeff, 1.0, y)

        # Path file output
        output_path = os.path.join(output_folder, 'filtered_boxcar_bandpass_fir.wav')
        ...
        plt.title('Sinyal Hasil Low-pass FIR Filter dengan Jendela Boxcar/Rectangle')
        plt.xlabel('Sample')
        plt.ylabel('Amplitude')
