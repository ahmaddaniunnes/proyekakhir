...
def elliptic_lowpass(cutoff, fs, order, rs, rp):
    nyquist = 0.5 * fs
    wp = cutoff / nyquist
    b, a = ellip(order, rp, rs, wp, btype='low', analog=False)
    return b, a

def lowpass_filter(data, cutoff, fs, order, rs, rp):
    b, a = elliptic_lowpass(cutoff, fs, order, rs, rp)
    y = lfilter(b, a, data)
    return y
...
def plot_signal(time, signal, filtered_signal, fs):
    ...
    plt.title('Sinyal Hasil Low-pass Elliptic (Cauer) Filter')
    plt.xlabel('Sample')
    plt.ylabel('Amplitude')
...
def process_audio(input_file, output_file, cutoff, order, rs, rp):
    # Read the audio file using librosa
    data, fs = librosa.load(input_file)

    # Apply the low-pass filter
    filtered_data = lowpass_filter(data, cutoff, fs, order, rs, rp)
...
if __name__ == "__main__":
    ...
    stop_attenuation = 30  # Minimum attenuation in stopband (dB)
    passband_ripple = 1  # Maximum ripple in passband (dB)

    process_audio(input_file, output_file, cutoff_frequency, filter_order, stop_attenuation, passband_ripple)
